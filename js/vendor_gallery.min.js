...
_Blog.changeTitle=function(){var currentTitle=document.title;document.title=currentTitle;};_Blog.toggleTheme=function(){const currentTheme=window.localStorage&&window.localStorage.getItem('theme')
...
var index=items.length;items.push(item);$figure.on('click',function(event){event.preventDefault();$(this).lightGallery({dynamic:true,download:false,showThumbByDefault:false,dynamicEl:items,index:index})});});});;$(document).ready(function(){$(window).scroll(function(){$(".top-scroll-bar").attr("style","width: "+($(this).scrollTop()/($(document).height()-$(this).height())*100)+"%; display: block;");});});